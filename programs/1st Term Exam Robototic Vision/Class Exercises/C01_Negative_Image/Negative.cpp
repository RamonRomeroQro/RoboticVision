...
void readhdr ()
{
     ...
     c = fgetc(infptr); // Advance pointer to first pixel of image
     printf("%d ",NCols);
...
void userdefined ()
{
	// Decalare pixel as a unsigned char to have values from 0 to 255
	unsigned char Pixel;
	...
	do {
		//fputc(255-Pixel,outfptr); // Save negative pixel in output image
		fputc(~Pixel,outfptr); // Save negative pixel in output image
		// Read next Pixel from input image
		Pixel = fgetc(infptr);
		// Check if its the last Pixel in image
	} while (!feof(infptr));
}  // end userdefined ()
