...
//----------------------------------------------------------------------------//
//        Enlarge the size of an input image by 4                             //
//----------------------------------------------------------------------------//

void userdefined ()
{
	unsigned char Pixel;			 // Read pixel from image
	unsigned char  Matrix[512][512];  // Input matrix
	unsigned char TMatrix[1024][1024];  // Tranformed matrix
    int col,row;
    
    // Read image in HardDrive and store it in a matrix
    for (row=0; row<512; row++)
    	for (col=0; col<512; col++){
    		Pixel = fgetc(infptr);
    		Matrix[col][row] = Pixel;
		}
		
	// Transform a 512 x 512 matrix into a 1024 x 1024 matrix
	// effectively enlarging its size by 4
	for (row=0; row<512; row++)
    	for (col=0; col<512; col++){
    		Pixel = Matrix[col][row];
    		TMatrix[col*2]  [row*2]   =Pixel;
    		TMatrix[col*2+1][row*2]   =Pixel;
    		TMatrix[col*2]  [row*2+1] =Pixel;
    		TMatrix[col*2+1][row*2+1] =Pixel;
	}
		
	// Saved transformed image to output file
	for (row=0; row<1024; row++)
    	for (col=0; col<1024; col++){
    		fputc(TMatrix[col][row],outfptr);
		}
