...
void negative()
{
  ...
  //Convolve image with mask
    for (ren = 1; ren < MRows-1; ren++)
      for (col = 1; col < NCols-1; col++) {
      	value          = ((mask[0][0] * mat[ren-1][col-1]  +
      	                   mask[0][1] * mat[ren  ][col-1]  +
      	                   mask[0][2] * mat[ren+1][col-1]  +
      	                   mask[1][0] * mat[ren-1][col  ]  +
      	                   mask[1][1] * mat[ren  ][col  ]  +
      	                   mask[1][2] * mat[ren+1][col  ]  +
      	                   mask[2][0] * mat[ren-1][col+1]  +
      	                   mask[2][1] * mat[ren  ][col+1]  +
      	                   mask[2][2] * mat[ren+1][col+1]) / dividefactor);
    	conv[ren][col] = value;
    	if      (value <   0) conv[ren][col] = 0;
    	else if (value > 255) conv[ren][col] = 255;
    	else                  conv[ren][col] = value;
      }
