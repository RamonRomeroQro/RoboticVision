...
void heading ()
{
     ...
     for (i=0; i<16; i++)
       ...
       printf("                       Template Author: RickWare            \n");
       printf("                   Editor: Ramon Romero - @ramonromeroqro   \n");
     for (i=0; i<3; i++)
       printf("                                    +                       \n");
       return;
} // end heading ()
...
void userdefined ()
{
	...
	unsigned  char **arrT = (unsigned char **)malloc(MRows/2 * sizeof(unsigned char *)); 
    for (i=0; i<MRows; i++) {
    	arrT[i] = (unsigned char *)malloc(NCols/2 * sizeof(unsigned char)); 
    }
    ...
    for (i = 0; i <  MRows; i++){
      for (j = 0; j < NCols; j++){
       arrT[i][j] = arr[i*2][j*2];  
      }
    }
    
    //Seleccionar unicamente las columnas y filas no par, donde  
